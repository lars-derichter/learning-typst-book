...
> A hands-on, cover-to-cover guide to the [Typst](https://typst.app) typesetting
> system — in the spirit of the old O'Reilly _Learning_ books: readable,
> occasionally funny, and thorough enough to actually finish the job.

## 📕 Read the finished book

**[Download the finished book as a PDF](learning-typst.pdf)**

The whole thing, typeset by its own pipeline: a cover, about 190 A4 pages, a
linked table of contents, styled admonition boxes, themed code, real math, and
every example rendered in place. If you want to _read_ Typst rather than watch a
book get built, start there — everything below is for the curious and the
contributors.
...
example in this book is a real, self-contained project in
[`examples/`](examples/) that compiles with **Typst 0.15.0** — no hand-waving,
no "left as an exercise" snippets that don't actually run.
...
9. [Set rules](book/09-set-rules.md) — changing the defaults, everywhere, at
   once.
10. [Show rules](book/10-show-rules.md) — intercepting and transforming anything
...
18. [Your own functions](book/18-your-own-functions.md) — building a personal
    style library.
19. [Templates](book/19-templates.md) — reusable document skeletons you can hand
    to other people.
...
24. [The Pandoc bridge](book/24-pandoc-bridge.md) — converting Markdown to
    Typst, a Lua filter for GitHub-style alerts, and typesetting _this whole
    book_ to PDF.
...
To (re)build and verify _every_ example at once:
...
- **Typst 0.15.0** or newer — https://typst.app/docs (web app needs nothing; the
  CLI is a single binary).
